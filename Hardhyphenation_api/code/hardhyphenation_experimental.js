...
Hyphenation = {
	...
	findAndFix : function (selector) {
		...
		if (previewElements.length > 0) {
			this.loopOverlap(previewElements);
		}
	},
	 ...
	 */
	checkContent : function (previewElement) {
			var txt = previewElement.innerHTML;
			// call a method to trim left and right the text
			txt = txt.trim();
			// get the width in pixels of the current element
			var eleWidth = previewElement.offsetWidth;
			...
			previewElement.innerHTML = fixedString;
	},	
	
	loopOverlap : function (previewElements) {
		var len = previewElements.length;
		var count = 0;
		var n = len % 4;
		if (n>0) {
			do {
				this.checkContent(previewElements[count]);
				count++;
			}
			while (--n);
		}
		...
		if (n>0) {
			do {
				this.checkContent(previewElements[count]);				
				this.checkContent(previewElements[count+1]);				
				this.checkContent(previewElements[count+2]);				
				this.checkContent(previewElements[count+3]);
				count +=4
			}
			while (--n);
		}
